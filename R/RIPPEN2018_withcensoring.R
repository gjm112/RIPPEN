...
library(dplyr)
fret <- subset(passPlays, season == 2018) %>%
...
gg <- ggplot(aes(x = X7, y = X3, label = name, colour = rippen), data = dist) +
    geom_point() +
    geom_abline(slope = rep(-7 / 3, 4), intercept = c(1 / 3, 2 / 3, 3 / 3, 4 / 3)) +
    scale_colour_gradient(high = "#D44500", low = "darkblue")
...
getmean <- function(x) {
    lapply(results_game[["2018"]][[x]], mean)
}

rippen2018_games <- data.frame(
    date = substring(row.names(rippen2018_games), nchar(row.names(rippen2018_games)) - 9, nchar(row.names(rippen2018_games))),
...
library(teamcolors)
prim <- c(
    teamcolors$primary[teamcolors$name == "New Orleans Saints"],
...
sec <- c(
    teamcolors$secondary[teamcolors$name == "New Orleans Saints"],
...
gg <- ggplot(aes(x = factor(name, levels = qbs), label = Date, y = rippen, col = factor(name, levels = qbs), fill = factor(name, levels = qbs)), data = subset(rippen2018_games, name %in% qbs), xlab = "QB") +
    scale_colour_manual("name", values = sec) +
    scale_fill_manual("name", values = prim) +
    geom_boxplot() +
    geom_point() +
    geom_point(aes(x = factor(name, levels = qbs), y = rippen), data = subset(rippen2018_season, name %in% qbs), col = "white", size = 1) +
    xlab("name")
...
ggplot(aes(x = date, y = rippen), data = df_brady_boot) +
    geom_boxplot(fill = teamcolors$primary[teamcolors$name == "New England Patriots"], col = teamcolors$secondary[teamcolors$name == "New England Patriots"]) +
    coord_flip() +
    geom_point(aes(x = date, y = rippen), data = subset(rippen2018_games, name == "T.Brady"), col = "white") +
    geom_point(aes(y = 2.68782, x = 17), col = "white")
...
GeneralSim <- function(qbs, years) {
    ...
    output <- data.frame(RIPPEN = 10 * unlist(lapply(results, function(x) {
        lapply(x, mean)
    })), passer = rep(qbs, each = 1), year = years) # Why are we repeating 8 times?
    return(output)
    # return(results)
}
...
g <- ggplot(data = output, aes(x = year, y = RIPPEN, colour = passer)) +
    geom_point() # This doesn't make sense for just our 2018 data. Let's try it with big boy data.
