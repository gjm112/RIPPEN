...
kicker <- data_list[[1999]] %>% filter(play_type=="field_goal") %>% select(field_goal_result, kick_distance)
...
pass <- dat %>% filter(play_type == "pass") %>% 
...
qbs <- pass %>% group_by(passer_player_name) %>% summarize(n = n()) %>% arrange(-n) %>% head(3) %>% pull(passer_player_name)
