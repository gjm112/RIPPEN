...
>>>>>>> 94f656a (task: Update manuscript (#2))
...
\section{Introduction}
In 1973, Don Smith, Seymour Siwoff, and Don Weiss developed the current formula for NFL passer rating. (https://www.profootballhof.com/news/nfl-s-passer-rating/).  Conceptually, the idea was to look at four passing categories -- completion percentage, passing yards, touchdowns thrown, and interceptions -- and award a quarterback 1 point for an average performance, 2 points for an elite performance, and 0 for a poor performance(http://www.donsteinberg.com/qbrating.htm).  Their rating across these four components could then be averaged.  Using data from the 1970 NFL season, the average completion percentage, average yards per attempt, average touchdown percentage, and average interception percentage were approximately 50\%, 7, 5\%, and 5.5\%.  

Using these numbers, formulas were derived to map dismal, average, and exceptional performances to approximately 0, 1, and 2, respectively.  For instance, for completion percentage the formula is $\frac{\frac{C}{A}-0.3}{0.2}$, which equals 0, 1, and 2 for a completion percentages of 30\%, 50\%, and 70\%, respectively.  Formulas for the other three components are defined similarly.  Each of the four components are bounded below by 0 and capped at 2.375 and then added together to get number between 0 and 9.5.  Then, this sum is divided by 6 and multiplied by 100.  The reason for this last adjustment was simply, as Don Steinberg explains it, to make the score ``more meaningful":   
>>>>>>> 94f656a (task: Update manuscript (#2))
...
=======
In college football, thre are no caps on each of the components, which means that the college football passer rating can vary from -731.6 to 1261.6. 

In this article, a new metric is proposed to replace passer rating.  The goal of the proposed measure is to use the exact same inputs as passer rating using only open source data and open source code, provide results on a meaningful scale in meaningful units, and to provide variability estimates of the measure.  
>>>>>>> 94f656a (task: Update manuscript (#2))
...
\begin{enumerate}
	\item Initialize: Set state to the 20 yard line, set downs to 1st and 10.
	\item Passing loop:
=======
\subsubsection{Drive Algorithm}
\begin{enumerate}
	\item Initialize: Set state to the 20 yard line, set downs to 1st and 10
	\item Passing loop: Until an end state is reached, simulate a pass play
>>>>>>> 94f656a (task: Update manuscript (#2))
	\begin{itemize}
		\item Check for completion
		\item On completion, simulate the yards gained
		\item On incomplete pass, check for interception
	\end{itemize}
<<<<<<< HEAD
	\item End states:
=======
	\item End states: For each simulated drive $j$, the outcome $D_j$ is determined is recorded in points
>>>>>>> 94f656a (task: Update manuscript (#2))
	\begin{itemize}
		\item Touchdown: if field position is >= 100, return $D_j$ = 7
		\item Field Goal: if 4th down is reached, simulate a field goal attempt and return $D_j$ = 3 if successful, 0 if not.
		\item Turnover: if interception, return $D_j$ =0
	\end{itemize}
\end{enumerate}
